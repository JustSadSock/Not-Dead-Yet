// game.js
//
// Полностью рабочий игровой цикл + небольшой фикс:
//   • player.angle = player.ang  (alias)  ← важно для перегенерации
//   • остальное — как в проверенной версии
...
// эти переменные меняет controls.js
window.joyDX = 0;
window.joyDY = 0;
...
function loop(now = performance.now()) {
  ...
  // движение
  const joyX = window.joyDX, joyY = window.joyDY;
  let dx = (+!!keys.d - +!!keys.a) + joyX,
      dy = (+!!keys.s - +!!keys.w) + joyY;

  if (dx || dy) {
    const m = Math.hypot(dx, dy);
    dx = dx * SPEED * dt / m; dy = dy * SPEED * dt / m;
    ...
    player.angle = player.ang;   // ← alias важен для regenerateChunksPreserveFOV
  }
