...
const FOV_DIST     = 6;
const FADE_RATE    = 1/4;          // затухание за 4 с
...
let C_W, C_H;
function onResize(){
  C_W = canvas.width  = innerWidth;
  C_H = canvas.height = innerHeight;
}
...
function loop(now=performance.now()){
  ...
  // 1) Предварительно создаём все соседние чанки (чтобы коллизии работали)
  ...
  if(regenTimer >= REGEN_PERIOD){
    ...
    if(toRegen.size){
      ...
      console.log(">>> После regen, ключи:", Array.from(gameMap.chunks.keys()));
      toRegen.clear();
    }
  }
...
function render(){
  ...
  ctx.fillStyle   = "#f00";
  ctx.beginPath();
  ctx.arc(player.x*TILE_SIZE, player.y*TILE_SIZE, TILE_SIZE*0.4,0,2*Math.PI);
  ctx.fill();
