...
//  КАРТА и игрок
// ——————————————————————————————
const gameMap = new window.GameMap(32);   // GameMap — из map.js (глобально)
...
// ——————————————————————————————
//  Встроенный FOV (без внешних файлов)
// ——————————————————————————————
...
function loop(now = performance.now()) {
  ...
  // периодическая перегенерация «забытых» чанков
  ...
  gameMap.chunks.forEach((chunk, key) => {
    ...
    for (let y = 0; y < gameMap.chunkSize; y++) {
      for (let x = 0; x < gameMap.chunkSize; x++) {
        ...
        ctx.fillStyle = (t === 'room') ? '#88b4ff' : '#6c9eff';
...
// expose globals для legacy-скриптов
window.ctx       = ctx;
