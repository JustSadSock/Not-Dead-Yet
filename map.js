  ...
  for(let i=0;i<N;i++){
    ...
    for(let yy=y;yy<y+h;yy++)
      for(let xx=x;xx<x+w;xx++)
        tiles[yy][xx]='room';
    rooms.push({
      minX:x, minY:y, maxX:x+w-1, maxY:y+h-1,
...
function carveCorridors(tiles,rooms){
  ...
  for(let i=1;i<rooms.length;i++){
    ...
    let x=A.cx, y=A.cy;
    // горизонтальный сегмент (2-пикселя)
    while(x!==B.cx){
      tiles[y][x]='hall';
      if(y+1<tiles.length) tiles[y+1][x]='hall';
      x+=Math.sign(B.cx-x);
    }
    // вертикальный сегмент (2-пикселя)
    while(y!==B.cy){
      tiles[y][x]='hall';
...
class GameMap {
  ...
  ensureChunk(cx,cy){
    ...
    const S=this.chunkSize,
          tiles=Array.from({length:S},()=>Array(S).fill('wall'));
